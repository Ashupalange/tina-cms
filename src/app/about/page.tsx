    ...
    <BaseLayout settings={settingsRes.data.settings}>
      ...
      <section className="woc-section min-h-[60vh] flex items-center justify-center text-center">
        <div className="woc-container max-w-4xl">
          <FadeUp>
            <h1 className="woc-h1 leading-tight text-[var(--color-primary)]">
              {data.headline || "We do not sell scents. We curate atmosphere."}
            </h1>
          </FadeUp>
        </div>
      </section>
        ...
        <div className="woc-container">
          <div className="flex flex-col md:flex-row gap-16 items-center">
            ...
            <FadeUp className="flex-1 space-y-6 md:px-8">
              ...
              <p className="woc-lead text-[var(--color-foreground)]">
                {data.originStory}
              </p>
            </FadeUp>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {data.pillars?.map((pillar: any, idx: number) => (
              <Stagger key={idx} className="text-center md:text-left space-y-4">
                ...
                <h3 className="woc-h3" style={{ color: "var(--color-dark-foreground)" }}>{pillar.title}</h3>
                <p style={{ color: "color-mix(in srgb, var(--color-dark-foreground) 70%, transparent)" }}>
                  {pillar.description}
                </p>
              </Stagger>
            ))}
